...
BQ_TABLE_NAME = "green_taxi_trips_raw"
GCS_PREFIX = "raw/green-taxi"
...
with DAG(
    ...
    def download_data(**kwargs):
        """Download and unzip the green Taxi data file."""
        logical_date = kwargs['logical_date'].strftime('%Y-%m')
        url = f"https://github.com/DataTalksClub/nyc-tlc-data/releases/download/green/green_tripdata_{logical_date}.csv.gz"
        output_file = f"/opt/airflow/data/green-tripdata-{logical_date}.csv.gz"
    ...
    upload_to_gcs = LocalFilesystemToGCSOperator(
        ...
        src="/opt/airflow/data/green-tripdata-{{ logical_date.strftime('%Y-%m') }}.csv.gz",
        dst=f"{GCS_PREFIX}/{{{{ logical_date.strftime('%Y') }}}}/green-tripdata-{{{{ logical_date.strftime('%Y-%m') }}}}.csv.gz",
    ...
    #     task_id="create_bq_table_green_taxi",
    #     dataset_id=BQ_DATASET_NAME,
    ...
    #                 OPTIONS (
    #                     format = 'CSV',
    #                     uris = ['gs://{GCS_BUCKET_NAME}/{GCS_PREFIX}/{{{{ logical_date.strftime('%Y') }}}}/green-tripdata-{{{{ logical_date.strftime('%Y-%m') }}}}.csv.gz']
    #                 );
    #             """,
    #             "useLegacySql": False,
    #         }
    ...
    #     task_id="create_bq_staging_table_green_taxi",
    #     configuration={
    #         "query": {
    #             "query": f"""
    ...
    #                     ))) AS unique_row_id,
    #                     "green-trip-data-{{ logical_date.strftime('%Y-%m') }}.csv" AS filename,
...
download_data >> create_bucket >> upload_to_gcs